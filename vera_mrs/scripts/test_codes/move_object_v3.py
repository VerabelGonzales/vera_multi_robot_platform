...
class VeraRobot:
    ...
    SECURITY_ZONE_RADIUS = 0.25  # 25 cm
    ...
    def navigation_on(self):
        ...
        while not rospy.is_shutdown() and self.euclidean_distance(1) >= VeraRobot.DISTANCE_TOLERANCE:
            ...
            elif self.check_collision('robot_2', 'robot_1'):

                if self.euclidean_distance(2) > self.euclidean_distance(1):
                    # Crear y enviar un mensaje de cancelación
                    cancel_msg = GoalID()  # Un mensaje vacío de GoalID cancelará la meta actual
                    self.cancel_goal_robot_2.publish(cancel_msg)
                    self.stop_robot_2()
            ...
            elif self.check_collision('robot_3', 'robot_1'):

                if self.euclidean_distance(3) > self.euclidean_distance(1):
                    # Crear y enviar un mensaje de cancelación
                    cancel_msg = GoalID()  # Un mensaje vacío de GoalID cancelará la meta actual
                    self.cancel_goal_robot_3.publish(cancel_msg)
                    self.stop_robot_3()
