...
class VeraRobot:
    ...
    def build_line(self, robot_id, offset_x, offset_y):
        ...
        if robot_id == 1:
            self.goal_1.pose.position.x = self.position_object.x + (offset_x)
            self.goal_1.pose.position.y = self.position_object.y + (offset_y)
            ...
            self.goal_2.pose.position.x = self.position_object.x + (offset_x)
            self.goal_2.pose.position.y = self.position_object.y + (offset_y)
            ...
            self.goal_3.pose.position.x = self.position_object.x + (offset_x)
            self.goal_3.pose.position.y = self.position_object.y + (offset_y)
    ...
    def velocity_controller_robot_1(self):
        ...
        point_x = self.pose_robot_1.x + VeraRobot.L * cos(self.pose_robot_1.theta)  # Coordenadas del punto externo en X
        point_y = self.pose_robot_1.y + VeraRobot.L * sin(self.pose_robot_1.theta)  # Coordenadas del punto externo en Y
        ...
        angular_velocity = (-control_x * sin(self.pose_robot_1.theta) / VeraRobot.L) + (control_y * cos(self.pose_robot_1.theta) / VeraRobot.L)
    ...
    def velocity_controller_robot_3(self, desired_x, desired_y):
        # Inicializar variables
        error_x = 0  # Error en la dirección X
        error_y = 0  # Error en la dirección Y
        linear_velocity = 0  # Velocidad lineal del robot
        ...
        desired_x = desired_x + distance_ahead  # Posición deseada en X (50 cm adelante)
        desired_y = desired_y  # Posición deseada en Y (misma posición Y)
    ...
    def run(self):
        ...
        # Variable para la verificacion del movimiento del objeto
        verification = self.pose_object.x
        
        #=================================================================
        command = input("Ingrese 'go' para inciar la tarea: ")
        if command == "go":

            self.build_line(1, -0.30, 0)
        ...
        while not rospy.is_shutdown():
            ...
            if (self.pose_robot_1.x >= 0.8):
                print("El robot ha movido el objeto a su objetivo")
                self.stop_robot_1()
                rospy.sleep(1)
                break

            if self.pose_object.x < verification + 0.1:
                print("El robot no puede mover el objeto")
                self.stop_robot_1()
                rospy.sleep(1)
                break
        ...
        command = input("Ingrese 'go' para madar a otro robot: ")
        if command == "go":

            self.build_line(1, -0.30, 0.26)
            self.build_line(2, -0.30, -0.26)

        while not rospy.is_shutdown():

            if self.result_data_robot_1.status.status == 3 and self.result_data_robot_2.status.status == 3:
        ...
        while not rospy.is_shutdown():
            ...
            goal_pose_robot_2.x = self.pose_robot_1.x 
            goal_pose_robot_2.y = self.pose_robot_1.y - 2*L
            goal_pose_robot_2.theta = self.pose_robot_1.theta
            ...
            if (self.pose_robot_1.x >= 0.8):
                print("Los robots han movido el objeto a su objetivo")
                self.stop_robot_1()
                self.stop_robot_2()
                rospy.sleep(1)
                break
            ...
            if self.pose_object.x < verification + 0.1:
                print("Los robots no pueden mover el objeto")
                self.stop_robot_1()
                self.stop_robot_2()
                rospy.sleep(1)
                break
        ...
        if command == "go":
            ...
            self.build_line(1, -0.30, 0.26)
            self.build_line(2, -0.30, 0.0)
            self.build_line(3, -0.30, -0.26)
        ...
        while not rospy.is_shutdown():
            ...
            goal_pose_robot_2 = Pose2D()
            goal_pose_robot_2.x = self.pose_robot_1.x 
            goal_pose_robot_2.y = self.pose_robot_1.y - L
            goal_pose_robot_2.theta = self.pose_robot_1.theta

            goal_pose_robot_3 = Pose2D()
            goal_pose_robot_3.x = self.pose_robot_1.x 
            goal_pose_robot_3.y = self.pose_robot_1.y - 2*L
            goal_pose_robot_3.theta = self.pose_robot_1.theta
            ...
            robot_3 = self.velocity_controller_robot_3(goal_pose_robot_3.x, goal_pose_robot_3.y)
        ...
        self.update_command_robot_1(-0.20, 0.0, 0.0)
        self.update_command_robot_2(-0.20, 0.0, 0.0)
        self.update_command_robot_3(-0.20, 0.0, 0.0)
        rospy.sleep(3)
        self.stop_robot_1()
        self.stop_robot_2()
        self.stop_robot_3()
