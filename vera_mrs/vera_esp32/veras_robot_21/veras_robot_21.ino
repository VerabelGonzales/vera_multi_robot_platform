...
const char* ssid = "Cudy-D4D2";
const char* password = "50133505ver";
...
Motor motor1(16, 17, 4, 0);
Motor motor2(27, 25, 26, 1);
Motor motor3(33, 32, 23, 2);
Motor motor4(5, 18, 19, 3);

// Asignacion de los pines a los encoders 
Encoder encoder1(21, 22);
Encoder encoder2(12, 14);
Encoder encoder3(13, 2);
Encoder encoder4(15, 0);
...
const double kp_m1 = 0.4887; 
const double ki_m1 = 1.391;
const double kd_m1 = 0.020958;

const double kp_m2 = 0.58671;
const double ki_m2 = 1.3784;
const double kd_m2 = 0.02589;

const double kp_m3 = 0.31867;
const double ki_m3 = 1.414;
const double kd_m3 = 0.012398;

const double kp_m4 = 0.49338;
const double ki_m4 = 1.4452;
const double kd_m4 = 0.021056;
...
const float RPM_CONVERSION_FACTOR = 60.0 / 1970;
const float RPM_MULTIPLIER = 10 * RPM_CONVERSION_FACTOR;

// Server settings
IPAddress server(192, 168, 10, 134);
...
// ROS /cmd_vel
ros::Subscriber<geometry_msgs::Twist> sub("robot_1/cmd_vel", &cmd_velCallback);

// ROS topic de los Tiks de los encoder de los motores: 1, 2, 3, 4
std_msgs::Int16MultiArray wheels_desired_rate_msg;
ros::Publisher wheels_desired_rate_pub("robot_1/wheel_ticks", &wheels_desired_rate_msg);
...
ros::Publisher ir_distances_pub("robot_1/ir_distances", &ir_distances_msg);
...
void setup()
{
  ...
  PID_M1.SetOutputLimits(10.0 * (255.0/500.0), 500.0 * (255.0/500.0));

  PID_M2.SetMode(AUTOMATIC);
  PID_M2.SetOutputLimits(10.0 * (255.0/500.0), 500.0 * (255.0/500.0));

  PID_M3.SetMode(AUTOMATIC);
  PID_M3.SetOutputLimits(10.0 * (255.0/500.0), 500.0 * (255.0/500.0));

  PID_M4.SetMode(AUTOMATIC);
  PID_M4.SetOutputLimits(10.0 * (255.0/500.0), 500.0 * (255.0/500.0));
...
void loop()
{
  ...
  if(current_millis - previous_millis_ir >= 100)
  {
  ...
  if(current_millis - previous_millis >= 100)
  {
    ...
    Input1 = rpm1;
    PID_M1.Compute();
    ledcWrite(motor1.pwm_channel, Output1);
    motor_speeds_msg.data[0] = rpm1;

    Input2 = rpm2;
    PID_M2.Compute();
    ledcWrite(motor2.pwm_channel, Output2);
    motor_speeds_msg.data[1] = rpm2;

    Input3 = rpm3;
    PID_M3.Compute();
    ledcWrite(motor3.pwm_channel, Output3);
    motor_speeds_msg.data[2] = rpm3;

    Input4 = rpm4;
    PID_M4.Compute();
    ledcWrite(motor4.pwm_channel, Output4);
